...
// Logika pre zobrazenie/skrytie tlačidla "Hore" pri akomkoľvek scrollovaní
...
function deletePlaylist(n) { 
    if (confirm(`Vymazať ${n}?`)) {
        ...
        const checkWin = setInterval(() => { if (delWin.closed) { clearInterval(checkWin); loadPlaylistHeaders(); } }, 500);
    }
}
...
async function submitErrorForm(e) {
    ...
    finally { btn.disabled = false; btn.innerText = "ODOSŤAŤ"; }
}

document.addEventListener('DOMContentLoaded', parseXML);const SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbyyrD8pCxgQYiERsOsDFJ_XoBEbg6KYe1oM8Wj9IAzkq4yqzMSkfApgcc3aFeD0-Pxgww/exec';
