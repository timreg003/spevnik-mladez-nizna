...
async function parseXML() {
    try {
        ...
        songs.sort((a,b) => formatSongId(a.displayId).localeCompare(formatSongId(b.displayId), undefined, {numeric:true}));
        filteredSongs = [...songs];
        renderAllSongs();
        loadPlaylistHeaders();
...
function savePlaylist() {
    ...
    setTimeout(() => { if(win) win.close(); alert("Uložené!"); location.reload(); }, 2000);
}
...
function sendErrorReport() {
    ...
    fetch(FORMSPREE_URL, {
        ...
        body: JSON.stringify({ pieseň: currentSong.title, správa: msg })
    }).then(() => {
